...
class _CalendarImportSheetState extends State<_CalendarImportSheet> {
  ...
  void initState() {
    super.initState();
    _categoryController.text = _defaultCalendarCategory;
  }
  ...
  String get _normalizedCategory =>
      _normalizeCalendarCategory(_categoryController.text);
...
class _CalendarSourceDraftFields extends StatefulWidget {
  ...
  @override
  State<_CalendarSourceDraftFields> createState() =>
      _CalendarSourceDraftFieldsState();
}

class _CalendarSourceDraftFieldsState
    extends State<_CalendarSourceDraftFields> {
  late final FocusNode _categoryFocusNode;
  bool _forceShowCategorySuggestions = false;
    ...
    _categoryFocusNode.addListener(() {
      ...
      setState(() {
        if (!_categoryFocusNode.hasFocus) {
          _forceShowCategorySuggestions = false;
        }
      });
    });
  }
  ...
  Widget build(BuildContext context) {
    final categories = {
      ...
      ...widget.existingCategories.map(_normalizeCalendarCategory),
    }.toList()..sort();
    final query = widget.categoryController.text.trim();
    ...
    final categoryOptions = <String>[
      if (query.isNotEmpty && !hasExactMatch) 'Create "$query"',
      ...filteredCategories,
    ];
    final showCategorySuggestions =
        _categoryFocusNode.hasFocus &&
        (_forceShowCategorySuggestions || query.isNotEmpty) &&
        categoryOptions.isNotEmpty;

    void applyCategorySelection(String selection) {
      const createdPrefix = 'Create "';
      final value =
          selection.startsWith(createdPrefix) && selection.endsWith('"')
          ? selection.substring(createdPrefix.length, selection.length - 1)
          : selection;
            ...
            children: [
              TextField(
                ...
                decoration: InputDecoration(
                  labelText: 'Category',
                  hintText: widget.defaultCategory,
                  suffixIcon: IconButton(
                    tooltip: 'Show category suggestions',
                    onPressed: () {
                      if (!_categoryFocusNode.hasFocus) {
                        _categoryFocusNode.requestFocus();
                      }
                      setState(() {
                        _forceShowCategorySuggestions =
                            !_forceShowCategorySuggestions;
                      });
                    },
                ...
                onChanged: (_) {
                  setState(() {
                    _forceShowCategorySuggestions = true;
                  });
                },
                onSubmitted: (_) {
                  setState(() {
                    ...
                    child: ConstrainedBox(
                      ...
                      child: ListView.builder(
                        ...
                        itemBuilder: (context, index) {
                          ...
                          final isCreate = option.startsWith('Create "');
                          return InkWell(
                            onTap: () => applyCategorySelection(option),
                            child: Padding(
...
class _CalendarPageViewState extends State<CalendarPageView> {
  ...
  Future<void> _renameSource(
    ...
    final normalizedCategory = _normalizeCalendarCategory(draft.category);
    if (trimmedName.isEmpty) return;
    if (trimmedName == source.displayName &&
        normalizedCategory == _normalizeCalendarCategory(source.category)) {
      return;
    }
            ...
            child: _CalendarSourceDraftFields(
              ...
              onSubmitted: () => Navigator.of(dialogContext).pop(
                _CalendarSourceDraft(
                  category: _normalizeCalendarCategory(categoryController.text),
